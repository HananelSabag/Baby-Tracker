...
async function buildVapidAuthHeader(audience: string): Promise<string> {
  ...
  // Import private key as JWK — the only reliable format for EC private keys in WebCrypto.
  // VAPID_PUBLIC_KEY is an uncompressed P-256 point: 0x04 || x(32) || y(32) (65 bytes total).
  // We extract x and y to build the JWK alongside the raw private key scalar d.
...
async function sendPush(sub: { endpoint: string; p256dh: string; auth: string }, payload: object): Promise<boolean> {
  try {
    ...
    console.error('[sendPush] error for endpoint', sub.endpoint.slice(0, 60), err)
    return false
  }
}
...
Deno.serve(async (req) => {
  try {
    ...
    console.log('[push-cron] Israel hour:', hour)
